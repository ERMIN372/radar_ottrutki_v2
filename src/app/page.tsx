...
export default async function DashboardPage({
  ...
  const [regions, summary, shops, top, weak, fill, runAttendance, runShowcase, snapshot] =
    await Promise.all([
      listRegions(),
      summaryByCriterion(day, p.region),
      shopStatuses(day, p.region),
      antiTop(p.from, p.to, 12, p.region),
      weakestCriteria(p.from, p.to, p.region),
      showcaseStats(day, p.region),
      lastRun('attendance'),
      lastRun('showcase'),
      snapshotInfo(),
    ]);
  ...
  const shopTotals = { green: 0, yellow: 0, red: 0 };
  for (const s of shops.values()) {
    if (s === 'green') shopTotals.green++;
    else if (s === 'yellow') shopTotals.yellow++;
    else if (s === 'red') shopTotals.red++;
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm muted">
            Статусы на {shortDate(day)}; тренды и анти-топ — за {shortDate(p.from)} — {shortDate(p.to)}
            {p.region ? ` · супервайзер ${p.region}` : ''}
          </p>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Tile title="Лавок в 🔴" value={shopTotals.red} total={shops.size} tone="red" />
        <Tile title="Лавок в 🟡" value={shopTotals.yellow} total={shops.size} tone="yellow" />
        <Tile title="Лавок в 🟢" value={shopTotals.green} total={shops.size} tone="green" />
        <ShowcaseTile fill={fill} totalShops={shops.size} />
      </div>

      {/* --- По критериям --- */}
      <section className="surface p-4">
        <h2 className="text-sm font-semibold">Лавки по критериям · {shortDate(day)}</h2>
            ...
            <table className="mt-3 w-full text-sm">
              <thead>
                <tr className="text-left text-xs muted">
                  <th className="pb-1.5 font-normal">Лавка</th>
                  <th className="pb-1.5 font-normal">Западает</th>
                  <th className="pb-1.5 pl-3 text-right font-normal">Витрина</th>
                  <th className="pb-1.5 pl-4 text-right font-normal">🔴</th>
                </tr>
              </thead>
              <tbody>
                {top.map((r) => (
                  <tr key={r.shop.code} className="border-t" style={{ borderColor: 'var(--border)' }}>
                    ...
                    <td className="py-1.5 pl-3 text-right tabular-nums whitespace-nowrap">
                      {r.fill == null ? '—' : `${Math.round(r.fill * 100)}%`}
                    </td>
        ...
        <section className="surface p-4">
          ...
          {!writable && (
            <p className="mt-2 text-xs muted">
              Данные загружены из снимка, собранного при сборке{' '}
              {new Date(snapshot.generatedAt).toLocaleString('ru-RU')}. На этом хостинге
              файловая система только для чтения, поэтому автосинк и кнопка обновления
              недоступны — см. README, раздел «Хостинг».
            </p>
          )}
        </section>
      </div>
...
function ShowcaseTile({
  ...
  return (
    <div className="surface p-4">
      <div className="text-xs muted">Среднее наполнение витрины</div>
