    ...
    <div className="flex flex-col gap-5">
      <div>
        ...
        <p className="mt-1 text-sm muted">
          Супервайзер: {shop.region ?? '—'} · код {shop.code} · период {shortDate(p.from)} —{' '}
          {shortDate(p.to)}
        </p>
      </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      ...
                      {item.origin === 'legacy' && (
                        <span className="text-[10px] muted" title="Статус взят из Витрины.xlsx как есть">
                          легаси
                        </span>
                      )}
                    </div>
            ...
            {day.people.length > 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-xs muted">
                      <th className="pb-1.5 font-normal">Сотрудник</th>
                      <th className="pb-1.5 font-normal">Должность</th>
                      <th className="pb-1.5 font-normal">Приход</th>
                      <th className="pb-1.5 font-normal">Откуда время</th>
                      <th className="pb-1.5 font-normal">Статус</th>
                      <th className="pb-1.5 font-normal">Примечание</th>
                    </tr>
                  </thead>
                  <tbody>
                    {sortPeople(day.people).map((person, i) => (
                      ...
                      >
                        ...
                        <td className="py-1.5 tabular-nums">{formatClock(person.arrivalMinutes)}</td>
                        <td className="py-1.5 text-xs muted">
                        ...
                        <td className="py-1.5 text-xs muted">{person.note ?? ''}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="mt-4">
                <p className="text-xs muted">
                  Сырых выгрузок за этот день нет — показаны статусы из легаси-таблицы, времена
                  отметок недоступны.
                </p>
